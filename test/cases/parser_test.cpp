...
TEST_CASE("Inline Comments - Semicolon comments") {
...
TEST_CASE("Inline Comments - Hash comments") {
    const char* content = "[comments]\n"
...
TEST_CASE("Quote Handling - Double quotes") {
...
TEST_CASE("Quote Handling - Single quotes") {
    const char* content = "[quotes]\n"
                          "basic = 'hello world'\n"
                          "spaces = '  spaced  '\n";
...
TEST_CASE("Quote Handling - Quotes protect comments") {
    const char* content = "[quotes]\n"
                          "semi = \"val;ue\"\n"
                          "hash = 'val#ue'\n"
...
TEST_CASE("Whitespace Handling - Whitespace trimming") {
...
TEST_CASE("Whitespace Handling - Preserved whitespace in quotes") {
...
TEST_CASE("Encoding - UTF-8 parsing") {
    tini_t* ini = tini_create(test_res_path("unicode.ini"));
    REQUIRE(ini != nullptr);
    tini_destroy(ini);
}

TEST_CASE("Edge Cases - Massive line") {
...
TEST_CASE("Edge Cases - Global keys") {
    ...
    if (global) {
        ...
        if (gk1) { REQUIRE(std::string(tini_key_get_value(gk1)) == "value1"); }
    }
...
TEST_CASE("Parser Config - No-section keys can be enabled explicitly") {
    const char* content = "global_key1 = value1\n"
                          "global_key2 = value2\n"
                          "[section]\n"
                          "key = value\n";
...
TEST_CASE("Parser Config - Colon delimiter can be enabled explicitly") {
...
TEST_CASE("Parser Config - Delimiter is strict") {
