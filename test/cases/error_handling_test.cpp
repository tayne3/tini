...
TEST_CASE("Error Codes - Last error after success") {
...
TEST_CASE("Error Codes - Error string") {
...
TEST_CASE("File Errors - Create nonexistent file") {
    tini_t* ini = tini_create(test_res_path("this_file_does_not_exist.ini"));
    REQUIRE(ini != nullptr);
    tini_destroy(ini);
}

TEST_CASE("File Errors - Save to invalid path") {
    tini_t* ini = tini_empty();
    REQUIRE(ini != nullptr);
...
TEST_CASE("Malformed Input - Missing section close") {
    ...
    if (ini) { tini_destroy(ini); }

    test_remove_temp_ini("malformed1.ini");
}

TEST_CASE("Malformed Input - Key without value") {
    ...
    if (ini) {
        ...
        if (sec) {
            ...
            if (key2) { REQUIRE(std::string(tini_key_get_value(key2)) == "value2"); }
        }
        tini_destroy(ini);
    }
...
TEST_CASE("Malformed Input - Duplicate sections") {
    const char* content = "[section]\n"
                          "key1 = value1\n"
                          "[section]\n"
                          "key2 = value2\n";
...
TEST_CASE("Malformed Input - Duplicate keys") {
    const char* content = "[section]\n"
                          "key = value1\n"
                          "key = value2\n";
...
TEST_CASE("NULL Handling - NULL ini operations") {
...
TEST_CASE("NULL Handling - NULL section operations") {
...
TEST_CASE("NULL Handling - NULL key operations") {
