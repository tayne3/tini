...
TEST_CASE("Section Iteration - Empty config") {
...
TEST_CASE("Section Iteration - Single section") {
    tini_t* ini = tini_empty();
...
TEST_CASE("Section Iteration - Multiple sections") {
    tini_t* ini = tini_empty();
...
TEST_CASE("Section Iteration - Section name") {
    tini_t* ini = tini_empty();
...
TEST_CASE("Section Iteration - NULL safety") {
...
TEST_CASE("Key Iteration - Empty section") {
    tini_t*         ini = tini_empty();
...
TEST_CASE("Key Iteration - Single key") {
    tini_t*         ini = tini_empty();
    tini_section_t* sec = tini_get_section(ini, "test");
...
TEST_CASE("Key Iteration - Multiple keys") {
    tini_t*         ini = tini_empty();
    tini_section_t* sec = tini_get_section(ini, "test");
...
TEST_CASE("Key Iteration - Key name") {
    tini_t*         ini = tini_empty();
    tini_section_t* sec = tini_get_section(ini, "test");
...
TEST_CASE("Full Enumeration - Nested iteration") {
    ...
    for (tini_section_t* sec = tini_first_section(ini); sec; sec = tini_section_next(sec)) {
        section_count++;
        for (tini_key_t* key = tini_section_first_key(sec); key; key = tini_key_next(key)) { total_keys++; }
    }
