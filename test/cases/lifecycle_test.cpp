...
TEST_CASE("Create/Destroy - Create from file") {
    tini_t* ini = tini_create(test_res_path("basic.ini"));
    REQUIRE(ini != nullptr);
    tini_destroy(ini);
}

TEST_CASE("Create/Destroy - Create file not found") {
    tini_t* ini = tini_create(test_res_path("nonexistent.ini"));
    REQUIRE(ini != nullptr);
    tini_destroy(ini);
}

TEST_CASE("Create/Destroy - Create empty") {
    tini_t* ini = tini_empty();
    REQUIRE(ini != nullptr);
    tini_destroy(ini);
}

TEST_CASE("Create/Destroy - Destroy NULL") {
    tini_destroy(nullptr);
}

TEST_CASE("Create/Destroy - Multiple create/destroy cycles") {
    for (int i = 0; i < 100; i++) {
        tini_t* ini = tini_empty();
        REQUIRE(ini != nullptr);
...
TEST_CASE("Save - Save then reload") {
...
TEST_CASE("Clear - Clear and reuse") {
    tini_t* ini = tini_empty();
    REQUIRE(ini != nullptr);
...
TEST_CASE("Clear - Clear idempotency") {
    tini_t* ini = tini_empty();
...
TEST_CASE("Load - Load merged (distinct)") {
    char path1[1024];
    char path2[1024];
...
TEST_CASE("Load - Load merged (overwrite)") {
    char path_def[1024];
    char path_ovr[1024];
...
TEST_CASE("Load - Load failure preservation") {
    char path_ok[1024];
